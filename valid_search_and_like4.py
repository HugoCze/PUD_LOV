...
class Search_And_Like:
    ...
    def get_homePage(self, path, comment):
        ...
        try:
            ...
            print(f"{date_time} - failed to get homePage - closing driver", file=open('SAL_LogFile.txt','a'))
    ...
    def search_comment(self, path, comment):
        ...
        for i in range(0, 34):
            try:
                ...
                try:
                    print(f"def search comment text: {comment_text}", file=open('SAL_LogFile.txt','a')) #; our type {comment}")
...
while True: 
    ...
    SAL.main("sylwester-marzen-tvp-black-eyed-peas-wystapili-w-teczowych-opaskach-tomasz-kammel-reaguje-zdjecia-6850623694637888a", "Pudelek do prania")
    end_time = time.time()
    total_time = end_time - start_time
    print(f"{date_time} - Time of looking the comment is equal to: {total_time} ",  file=open('SAL_LogFile.txt','a'))